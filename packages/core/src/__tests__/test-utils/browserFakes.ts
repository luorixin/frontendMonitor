...
const fakeIndexedDbDatabases = new Map<string, FakeIndexedDbState>()
    ...
    const resolvedKey =
      key !== undefined
        ...
        : this.keyPath &&
            value &&
            typeof value === "object" &&
            this.keyPath in (value as Record<string, unknown>)
          ? (value as Record<string, unknown>)[this.keyPath] as IDBValidKey
          : undefined

    if (resolvedKey === undefined) {
      request.fail("Missing key")
      return request as unknown as IDBRequest<IDBValidKey>
    }

    this.values.set(String(resolvedKey), value)
    request.succeed(resolvedKey)
    return request as unknown as IDBRequest<IDBValidKey>
  }
}
...
class FakeIDBTransaction {
  constructor(
    ...
    private readonly keyPaths: Map<string, string | undefined>
  ) {}
...
export const FakeIndexedDB = {
  open(name: string, version?: number): IDBOpenDBRequest {
    ...
    queueMicrotask(() => {
      let state = fakeIndexedDbDatabases.get(name)
      const requestedVersion = version ?? 1
      const isNewDatabase = !state
      const shouldUpgrade =
        isNewDatabase || requestedVersion > (state?.version ?? 0)
      ...
      if (!state) {
        ...
        fakeIndexedDbDatabases.set(name, state)
      } else if (requestedVersion > state.version) {
        state.version = requestedVersion
      }

      const database = new FakeIDBDatabase(name, state.stores, state.keyPaths)
